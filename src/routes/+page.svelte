<script lang="ts">
    ...
    let gewichtungen = $derived.by(() => {
        ...
        let indizesNochZuGewichten = muendlicheNoten.map((note,index) => index).filter((index) => muendlicheNoten[index])

        while(indizesNochZuGewichten.length) {
            let aktuellerSchnitt = schnittBerechnen(schriftlicheNoten, muendlicheNoten, gewichtungen)
            let notenNochZuGewichten : Array<number> = indizesNochZuGewichten.map((index) => muendlicheNoten[index])
            let besteNote = Math.min(...notenNochZuGewichten)
            let indexDerBestenNote  = muendlicheNoten.findIndex((el,i) => el === besteNote && indizesNochZuGewichten.includes(i))
	...
	function schnittBerechnen(schriftlicheNoten: number[], muendlicheNoten: number[], gewichtungen: (null|1|2)[]) {
...
<table>
	<tbody>
        ...
        <tr class="gewichtungen">
            ...
            <td class={gewichtung == 2 ? 'doppelt' : ''}><div>{gewichtung} <br> <small>{gewichtungText(gewichtung)}</small></div></td>
        {/each}
...
<p>
Schnitt der kleinen Leistungnachweise:   <b> {schnitt.toFixed(2)} </b>
</p>
...
<style>
	...
	* {
    ...
    div.schriftlicheNoten, div.muendlicheNoten {
        padding: 20px 0;
    }
